...
for i in range(1,len(datos_elec)):
    ...
    if(mismoUser & mismaSesion):
        ...
        if(elaboration):
            userId_log.append(userId[i])
            sesion_log.append(sesion[i])
            actividad_log.append('ASSESSMENT')
            timestamp_log.append(timestamp[i])
        if(studyingTactics):
            userId_log.append(userId[i])
            sesion_log.append(sesion[i])
            actividad_log.append('PLAN')
            timestamp_log.append(timestamp[i]-1)
    else:       
...
logSesionesSRL_todo.to_csv('procesoSesionesSRL3.csv', index = False, columns = ['userId', 'sesion', 'timestamp', 'actividad', 'curso','aprueba', 'HighSRL'])